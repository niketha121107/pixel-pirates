...
export const Sidebar = () => {
    ...
    return (
        <aside className="hidden lg:flex flex-col w-64 fixed left-0 top-16 bottom-0 bg-white/90 backdrop-blur-md border-r border-pink-100 pt-4 pb-4 z-30">
            {/* Dashboard Nav Link */}
                ...
                <div className="space-y-1">
                    {recentSearches.map((item, i) => (
                        <motion.button
                            key={i}
                            whileHover={{ x: 4 }}
                            onClick={() => navigate(`/topic?q=${encodeURIComponent(item)}&difficulty=${difficulty}`)}
